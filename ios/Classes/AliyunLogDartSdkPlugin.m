...
//    void * slf = (__bridge_retained void *)(self);
//    LogProducerClient *_logProducerClient = [[LogProducerClient alloc] initWithLogProducerConfig:_logProducerConfig callback:on_send_done_function_in_flutter userparams:slf];
...
void on_send_done_function_in_flutter(const char * config_name, log_producer_result result, size_t log_bytes, size_t compressed_bytes, const char * req_id, const char * error_message, const unsigned char * raw_buffer, void *user_param) {
    if (user_param == NULL) {
        NSLog(@"user_param is NULL");
        return;
    }

    if (![(__bridge id)user_param isKindOfClass:[AliyunLogDartSdkPlugin class]]) {
        return;
    }
    ...
    __block NSString* configName = [NSString stringWithUTF8String:config_name];
    __block NSString* errorMessage = [NSString stringWithUTF8String:error_message];

    dispatch_async(dispatch_get_main_queue(), ^{
        [plugin onLogSendDone:configName
               resultCode:result
