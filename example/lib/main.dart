...
class _MyAppState extends State<MyApp> {
  ...
  AliyunLogDartSdk? _aliyunLogSdk2;
  String _consoleText = '';
  ...
  void _initProducerWithPersistent() async {
    if (null != _aliyunLogSdk2) {
      print('producer has been inited. please restart app.');
      return;
    }
    LogProducerConfiguration configuration = LogProducerConfiguration(
        endpoint: 'https://cn-hangzhou.log.aliyuncs.com', project: 'yuanbo-test-1', logstore: 'test2');
    configuration.accessKeyId = '';
    configuration.accessKeySecret = '';
    ...
    _aliyunLogSdk2 = AliyunLogDartSdk();
    LogProducerResult result = await _aliyunLogSdk2!.initProducer(configuration);
    print('init producer: ${result.name}');

    _aliyunLogSdk2!.setLogCallback((resultCode, errorMessage, logBytes, compressedBytes) {
      print('log send result: ${resultCode.name}');
    });
  ...
  void _sendLog() async {
    ...
    result = await _aliyunLogSdk2!.addLog({
      'str': 'str value2',
      'int': 12,
      'double': 12.12,
      'boolean': true,
    ...
    print('add log resutlt2: ${result.name}');
  }
